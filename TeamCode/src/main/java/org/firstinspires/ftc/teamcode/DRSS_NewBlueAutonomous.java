...
@Autonomous(name = "DRSS NewBlueAutonomous", group = "DRSS")

public class DRSS_NewBlueAutonomous extends LinearOpMode{
...
//      rightMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
    ...
    private void turn(int gyroTarget){
        ...
        double lPower  = Range.clip(2 * (gyro.getIntegratedZValue() - gyroTarget), -0.4, 0.4);
        double rPower = - Range.clip(2* (gyro.getIntegratedZValue() - gyroTarget), -0.4, 0.4);
    ...
    private void shoot(){
        if(runTime.time() < 3500) {
            ...
            cannonSeal.setPosition(opticalDistanceSensor.getLightDetected() <= 0.0015 ? 1.0:0.75);
        } else {
